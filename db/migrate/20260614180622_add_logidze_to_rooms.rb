class AddLogidzeToRooms < ActiveRecord::Migration[8.1]
  def change
    add_column :rooms, :log_data, :jsonb

    reversible do |dir|
      dir.up do
        execute <<~SQL
          CREATE TRIGGER "logidze_on_rooms"
          BEFORE UPDATE OR INSERT ON "rooms" FOR EACH ROW
      ...
      dir.down do
        execute <<~SQL
          DROP TRIGGER IF EXISTS "logidze_on_rooms" on "rooms";
        SQL
      end
    end
    reversible do |dir|
