    ...
    class QuantityAdjuster < Components::Base
      ...
      def view_template
        div(
          ...
          data: { controller: "quantity-adjuster", quantity_adjuster_min_value: @min },
          **@attrs
        ) do
          step_button(:decrease, Components::Icons::Minus)
          span(
            class: "min-w-10 text-center text-body-lg font-bold tabular-nums",
            data: { quantity_adjuster_target: "display" }
          ) { @value.to_s }
          input(
            type: "hidden", name: @name, value: @value,
            data: { quantity_adjuster_target: "input" }
          )
      ...
      def step_button(action, icon)
        button(
          ...
          data: { action: "quantity-adjuster##{action}" },
