...
module FindLists
  ...
  # row's undo. Same pin guard as MarkFound (the row is the only caller, so a
  # missing pin means a stale form or a crafted URL); delegates to
  # Items::RestoreToBox, which owns the writable-Move invariant and emits
  # item.restored — no find_list.* event on top.
