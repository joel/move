...
module Items
  ...
  class GenerateImage < BaseAction
    ...
    def build_media(item, result)
      ...
      # Generated bytes bypass ImageNormalizer (trusted provider output, kept
      # verbatim), so stamp the blur-up preview here or generated photos would
      # permanently lack one (#681 — images:lqip is a one-off backfill). nil
      # (undecodable/no libvips) just omits the key; ItemCard falls back to the
      # plain placeholder. Dimensions still come from the async AnalyzeJob —
      # generated photos never enter the gallery lightbox (not_generated).
      lqip = ImageNormalizer.lqip_base64(result.image_bytes)
      media.image.attach(
