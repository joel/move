...
module DemoData
  ...
  class Provision < BaseAction
    ...
    def create_move(owner)
      Moves::Create.new.call(
        params: {
    ...
    # Raises on failure (image attach, storage, …); the job is the error boundary —
    # it marks the org "failed" and reveals a fallback card rather than crashing.
    def build_contents(move)
