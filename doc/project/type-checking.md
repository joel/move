...
Move type-checks the **whole app** — the actions layer (root + every pack —
#519), the models (with real schema-derived types — #521), the controllers
(with typed route helpers: a renamed route is a NoMethod error at every stale
call site — #523), and the Phlex views/components (#525) — with
...
lowest-friction layer to type. Growth was staged like the Packwerk migration:
pack-by-pack, each addition a `check` line in the Steepfile — actions in #519,
the models (real model types) in #521, the controllers (typed route helpers)
in #523, and finally the Phlex views/components in #525. Nothing is out of
scope anymore.

Phlex-layer notes (#525): the gem ships no RBS, so the tag DSL resolves via
**`sig/phlex.rbs`, 100% GENERATED by introspecting the installed gem's element
registries** — regenerate after a phlex bump with:

```bash
docker exec -i move-app-dev bin/rails runner - <<'RUBY'
# (see git history of sig/phlex.rbs for the full generator — it rewrites the
# file from Phlex::HTML::StandardElements/VoidElements + SVG::StandardElements
# with the uniform `(*untyped, **untyped) ?{ (*untyped) -> untyped } -> untyped`
# shape per tag)
RUBY
```

Hand-curated Phlex surface (SGML core, the phlex-rails helper-adapter modules,
`Attached`-style gaps) lives in `sig/phlex_rails.rbs`; component `def self.`
declarations in `sig/component_singletons.rbs`. Gotchas unique to this layer:

- **A def whose name shadows an HTML tag** (`def caption`, `def summary`)
  inherits the generic tag signature and fails `MethodArityMismatch` — its own
  `#:` annotation resolves the override.
- **Block-forwarding defs** (`def x(&)` passing `&` to a tag) must declare the
  optional block: `#: () ?{ (*untyped) -> untyped } -> untyped`.
- `Rails.application.routes.url_helpers` is typed as an anonymous module by
  the community sigs — bind it to a local with `#: untyped`.
- `app/views/layouts/chrome_head.rb` is a view module mixin (the concern
  problem) — excluded; its surface is declared in `sig/concerns.rbs`.
- **Path-scoped `steep check <paths>` is unreliable for the component tree**
  (it can match zero files and print a vacuous "No type error detected", or
  fail spuriously on cross-file inline declarations) — judge view/component
  work only by the **whole-project** run.
