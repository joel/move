...
| Error-report egress | Auth material (magic-link `key` — in query strings, Sequel-literalized SQL, and mailer-job args — session/remember cookies, MCP Bearer tokens, outbound OAuth bodies) shipped to Sentry with the request context (`send_default_pii`) | Fail-closed `before_send` scrub: cookies + `Authorization` + job `arguments` + breadcrumb `body`/`query` dropped; query string + form/JSON bodies through `ActiveSupport::ParameterFilter` (unparseable ⇒ `[FILTERED]`); `:sql` removed from breadcrumb payloads; init gated on DSN presence + `enabled_environments=%w[production]`; DSN an optional Doppler secret, never committed (#528) | `config/initializers/sentry.rb`, `spec/config/sentry_spec.rb` |
