...
| Direct upload to R2 (#572) | Unauth'd/abusive presign; raw metadata-bearing bytes served as-is; cross-Move attach | Presign endpoint is behind the capture guards (membership + writable + capturable) and rejects over-cap size / non-image declared type before minting a URL; the returned `signed_id` is **Move-purpose-bound** (`web_media_upload/<tenant>/<move>`) so a leaked id can't attach cross-Move; the browser's raw bytes land in R2 first, then `ImageNormalizer` runs in `IngestJob` as a **mandatory post-upload step** — re-sniff type, **strip EXIF/GPS**, size-cap, transcode — and **replaces** the master, so no raw metadata-bearing object is ever served or transformed; abandoned presigns reaped by `PurgeAbandonedUploadsJob` | `app/controllers/captures_controller.rb#direct_upload`, `packs/captures/app/actions/captures/start_ingest.rb`, `packs/captures/app/services/image_normalizer.rb` |
