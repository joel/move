...
| File storage | `config/storage.yml`, `config/deploy.yml`, `bin/cli-files/storage-cmd/storage_service.rb` | Active Storage; dev/test = Disk, prod = the **shared host-wide SeaweedFS S3** gateway (also used by sibling apps) via move's own `move` bucket (`STORAGE_ENDPOINT=http://seaweedfs:8333`, `force_path_style`). Local `bin/cli storage start` exposes the SeaweedFS Web UI at `https://storage.move-easy.docker` (filer `:8888`) and the app bucket at `https://bucket.move-easy.docker` (S3 `:8333` with `/move` prefixed). Images served through **proxy URLs** (internal endpoint never exposed). Media tables are per-tenant (not Apartment-excluded) |
