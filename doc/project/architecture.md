...
flowchart LR
  ...
  W -->|"Cache API<br/>(token/exp stripped;<br/>format + encoding version kept)"| EDGE[("Per-colo edge cache")]
```
...
| Media edge transform | `workers/media-transform/` (Worker), `packs/captures/app/services/media_variants/transform_url.rb` (Rails minter), `config/initializers/media_transform.rb` (#572) | Cloudflare Worker on Custom Domain `media.<zone>` (edge, **not** the Tunnel — see §1a) resizes the R2 master on demand (`thumb` 400 / `detail` 1600, `scale-down`; avif/webp/jpeg by `Accept`, encoded at quality 80 (#679); edge-cached with a token-stripped cache key carrying an encoding version — bump `CACHE_VERSION` when quality/format logic changes). Access = short-lived HMAC token over `blob_key\|size\|exp` under a **dedicated** `MEDIA_TRANSFORM_SECRET` (never `secret_key_base`), verified constant-time in the Worker; R2 bucket stays private (Worker R2 binding). `MEDIA_TRANSFORM_HOST`/`_SECRET` are prod-only (Doppler) — **dev/test have no Worker and fall back to the same-origin master proxy**. Deploys independently of Kamal (`wrangler deploy`); must be live before the Rails config ships |
| Web capture upload | `app/controllers/captures_controller.rb#direct_upload`, `packs/captures/app/actions/captures/start_ingest.rb`, `app/javascript/controllers/capture_upload_controller.js` (#572) | In prod the browser **direct-uploads** the (client-downscaled) photo straight to R2 via a presigned PUT — a tenant+membership-gated presign endpoint validates size/type and returns a **Move-scoped `signed_id`**; `StartIngest` reserves that blob and enqueues `IngestJob`. The raw bytes never proxy through the app. On any direct-upload failure the client falls back to the **server-proxied POST** (`file`), which is also the dev/test path (`config.x.direct_upload_enabled` off). Either way `IngestJob`/`ImageNormalizer` is the post-upload authority (below). The MCP path stays app-proxied (`McpUploadsController`, §7) |
| Image optimisation | `app/services/image_normalizer.rb`, `app/models/media.rb`, `lib/tasks/images.rake` (#299) | Every upload (web + MCP, one choke point) is decoded, auto-rotated, down-scaled to a **≤2048px JPEG master** (`MASTER_IMAGE_EDGE`, Q85, EXIF/GPS stripped, alpha flattened) before attach — the phone original is **never stored**. With direct upload the raw bytes land in R2 first, so this runs as a **post-upload** step in `IngestJob` that re-reads and **replaces** the master (never bypassed). Display surfaces serve sized images off the master via the **Cloudflare-edge transform Worker** (`:thumb` 400px / `:detail` 1600px — see the "Media edge transform" row and §1a; #572), not the master directly. (The old in-app Active Storage variant pipeline — `Media#image` variant declarations + `MediaVariants::Prewarm`/`PrewarmJob`/subscriber + `images:prewarm`/`images:repair` — was **removed** with the edge cutover; #572. `images:cleanup_variants` purges the orphaned variant records/objects.) `media.optimized_at`/`original_byte_size` track the backfill; `images:optimize` re-encodes pre-existing blobs across all tenants (idempotent) and reclaims storage. Recognition is unaffected — it re-downscales to 1536px itself |
