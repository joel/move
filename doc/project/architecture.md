...
| Auth | `app/misc/rodauth_main.rb` | passwordless (passkey / email-link / Google); **all tables `Sequel[:public][:…]`**; a personal tenant is provisioned both post-verify and after a Google (OmniAuth) login (`after_login` → `ensure_personal_organization`, idempotent). Google sign-in is gated on `GOOGLE_CLIENT_ID` — see [`new-app-recipe.md`](new-app-recipe.md) §6c. **Google One Tap** (`app/controllers/google_one_tap_sessions_controller.rb`) is **login-only** — it never creates an account from the tokeninfo-verified id_token; a brand-new user is bridged into the account-creating OAuth flow (`/login?via=google` auto-submits to `/auth/google`, whose callback runs `omniauth_create_account?`). A `one-tap-reset` Stimulus controller (mounted via `ChromeHead#body_controllers` **only when signed in**, so it can't loop on the logged-out bridge page) clears the `no_account` suppression flag so One Tap re-prompts after sign-out; the FedCM cooldown after that is browser-enforced |
