...
| `wrangler deploy` warns `Unexpected fields found in images field: routes` and prompts for a `*.workers.dev` subdomain | in `wrangler.toml`, a top-level key (`routes`, `workers_dev`) sits **after** a `[[table]]`/`[table]` header, so TOML parses it as a field of that table — the Custom Domain is silently dropped (#572) | put all top-level keys **before** any table header; `workers_dev = false` keeps the Worker on the Custom Domain only |
| edge media Worker returns **500** for every otherwise-valid request (but 403/400 on bad token/size) | `MEDIA_TRANSFORM_SECRET` isn't bound to the Worker — `crypto.subtle.importKey` throws before verify. The Worker secret (`wrangler secret put`) is **separate** from the Doppler/GitHub secret (that's for the Rails deploy) | set the Worker secret; guard its presence + fail closed. `workers/media-transform/README.md` |
| media `<img>` all blocked by CSP in prod even though `img-src` was widened | `content_security_policy.rb` closure-captures at boot and sorts **before** `media_transform.rb`, so `config.x.media_transform_host` is still unset there | read `ENV[...]` **directly** in the CSP initializer (like `GOOGLE_CLIENT_ID`), not `config.x` (#572) |
| an authenticated user can mint R2 presigned PUTs bypassing the app's guards | Rails' built-in `POST /rails/active_storage/direct_uploads` stays mounted + **unauthenticated** even after you add a guarded presign endpoint | `config.active_storage.draw_routes = false` in prod (safe when prod uses no stock AS routes — media via the Worker, uploads via the custom presign) (#572) |
| CI "model signature freshness" step fails after adding a route | route-helper sigs in `sig/rbs_rails/path_helpers.rbs` drift on any route change | `RAILS_ENV=test bin/rails rbs_rails:all` and commit the regenerated sig (§7 rule) |
...
| Service worker has a literal `<%= … %>` in it / cache version never busts | the SW template is `app/views/pwa/service-worker.**js**` (no `.erb`), so ERB tags are served verbatim — `rails/pwa#service_worker` doesn't ERB-process a non-`.erb` template | rename to `service-worker.js.**erb**` so `CACHE_VERSION = "<%= Rails.application.config.assets.version %>"` renders; bumping `config.assets.version` then wipes the SW caches on the next activate (#395) |
| Service worker serves a stale authed/unauthed shell after login/logout | the SW cached an HTML navigation | **never cache navigations** — the auth layer (passwordless Rodauth + Apartment elevator + cross-host handoff) is fragile. SW strategy: **network-first** for `request.mode === "navigate"` → precached `/offline.html` only on network failure; **cache-first** for fingerprinted `/assets/` + static icons; **network-only** for auth endpoints, `text/vnd.turbo-stream.html` responses, `/cable`, `/manifest`, the SW itself, and all non-GET (#395) |
