...
| embeddings nil right after flipping `EMBEDDING_PROVIDER: openai` | env selector set before `OPENAI_API_KEY` reached the env | add the key to Doppler `move/prd` **first**, then deploy; backfill with `kamal app exec --reuse 'bin/rails search:reindex'` |
