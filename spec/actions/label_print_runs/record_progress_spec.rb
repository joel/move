...
RSpec.describe LabelPrintRuns::RecordProgress do
  let(:move) { create(:move) }

  it "sets the absolute completed count on an active run" do
    run = create(:label_print_run, :processing, move:, total_count: 10, completed_count: 0)
  ...
  it "is a no-op on a finished run (a late call after completion)" do
    run = create(:label_print_run, :completed, move:, total_count: 5)
