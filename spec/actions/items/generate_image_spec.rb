...
RSpec.describe Items::GenerateImage do
  ...
  it "generates an image, attaches it as the item's source_media, and emits the event" do
    ...
    aggregate_failures do
      ...
      # Generated bytes bypass ImageNormalizer, so the action stamps the
      # blur-up preview itself (#681) — without this the card would show the
      # grey placeholder forever (images:lqip is a one-off backfill).
      expect(item.source_media.image.blob.metadata["lqip"]).to match(%r{\A[A-Za-z0-9+/]+={0,2}\z})
