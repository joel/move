...
7. **Static types → RBS + Steep, actions-first.** The whole actions layer —
   `app/actions/**` and every pack's `packs/*/app/actions/**` — carries inline
...
2. **Types:** `bundle exec steep check --no-daemon --severity-level=error` (scope: the whole actions layer — `app/actions` + `packs/*/app/actions` + `sig/`; see [`doc/project/type-checking.md`](doc/project/type-checking.md)).
