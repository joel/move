...
- **Self-healing pipeline** ([`doc/project/self-healing.md`](doc/project/self-healing.md)): the `Self-Healing` workflow polls Sentry every 30 min and can open `autofix` issues, generate fix PRs, and (once `SELF_HEALING_AUTONOMY_ENABLED` is set) auto-merge scored+reviewed fixes — each merge deploying like any other. Kill switch: repo variable `SELF_HEALING_ENABLED`; circuit breaker: any open `self-healing-halt` issue. **Never put Sentry event messages/breadcrumbs into issues, PRs, prompts, or logs** — all Sentry data must flow through the whitelist reducer `script/self_healing/sentry_fetch.rb` (security-model.md trust boundary #5).
